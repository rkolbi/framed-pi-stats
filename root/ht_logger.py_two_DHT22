...
try:
    ...
    if os.stat('/var/www/html/enviro.csv').st_size == 0:
            f.write('Date,Time,TempI,TempE,HumE\r\n')
...
while True:
    ...
    if humidityI is not None and temperatureI is not None and humidityE is not None and temperatureE is not None:
        ...
        f.write('{0} {1},{2:0.1f},{3:0.1f},{4:0.1f}%\r\n'.format(time.strftime('%m/%d/%y'), time.strftime('%H:%M:%S'), ((temperatureI * 1.8) + 32), ((temperatureE * 1.8) + 32), humidityE))
        f.flush()

        f2 = open("/var/www/html/latest.php", "w")
        f2.write('Saturn: {0:0.1f} / Tent: {1:0.1f}@{2:0.1f}%\r\n'.format(((temperatureI * 1.8) + 32), ((temperatureE * 1.8) + 32), humidityE))
        f2.close()
        f.flush()
